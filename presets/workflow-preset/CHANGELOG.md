...
- Split implement Worker prompts into Implementation Worker and Code Review Worker roles, and added manual worker queue fallback when isolated subagents are unavailable.
- Migrated product, design, provider, HTML SSOT, and structured IR intake ownership out of the workflow preset and into the external `spec-kit-intake` extension; the preset now consumes external intake refs, visual SSOT refs, HTML SSOT refs, structured IR refs, readiness inputs, and provider blocker status while retaining `spec.md` projection and Visual Fidelity readiness.
- Clarified that missing provider/intake evidence remains `[BLOCKED: PROVIDER_EVIDENCE]`, missing product decisions remain `[NEEDS CLARIFICATION]`, and downstream planning/tasks must not define screenshot comparison, visual diff, baseline capture, or final visual review work.
